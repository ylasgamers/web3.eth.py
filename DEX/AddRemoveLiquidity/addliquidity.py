...
ctypes.windll.kernel32.SetConsoleTitleW("Add Liquidity DEX AIO Blockchain Network")
print('')
print('Add Liquidity DEX AIO Blockchain Network')
print('This Support DEX PancakeSwap V2, Uniswap V2, QuickSwap V2, Sushiswap, Camelot, & Other')
print('')
...
contract_address = web3.toChecksumAddress(input("Enter Contract Address Router DEX : "))
...
tokenSupply = token_contract.functions.totalSupply().call()
...
inputamount_token = float(input("Enter Amount Of Token You Want To Add Liquidity : ")) #ex 1 / 0.1 / 0.001 / 0.0001 / 0.00001
amount_token = web3.toWei(float(inputamount_token), 'ether')
inputamount = float(input("Enter Amount Of BNB/ETH/MATIC/OTHER You Want To Add Liquidity : ")) #ex 1 / 0.1 / 0.001 / 0.0001 / 0.00001
amount = web3.toWei(float(inputamount), 'ether')
deadline = int(time.time()) + 1000000

#estimate gas limit contract
nonce2 = web3.eth.getTransactionCount(sender)
gas_tx = contract.functions.addLiquidityETH(tokenaddr, amount_token, amount_token, amount, sender, deadline).buildTransaction({
    'chainId': chainId,
    'from': sender,
    'value': amount,
...
#calculate transaction fee
print('')
tokenwei = web3.fromWei(amount_token, 'ether')
amountwei = web3.fromWei(amount, 'ether')
gasPrice = web3.fromWei(web3.eth.gas_price, 'gwei')
Caclfee = web3.fromWei(gasPrice*gasAmount, 'gwei')
print('Transaction Fee :' ,Caclfee, 'ETH/BNB/MATIC/OTHER')
print('Processing Added :' ,tokenwei, tokenSymbol, '&' ,amountwei, 'ETH/BNB/MATIC/OTHER To Liquidity Pair')

token_tx = contract.functions.addLiquidityETH(tokenaddr, amount_token, amount_token, amount, sender, deadline).buildTransaction({
    'chainId': chainId,
    'from': sender,
    'value': amount,
    'gas': gasAmount,
...
print('')
print('Transaction Success Liquidity Added! TX-ID Copied To Clipboard')
print(txid)
pc.copy(txid)
