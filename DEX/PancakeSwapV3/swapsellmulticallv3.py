...
ctypes.windll.kernel32.SetConsoleTitleW("Swap Sell On DEX PancakeSwap V3")
print('')
print('Swap Sell On DEX PancakeSwap V3 With Auto Slippage')
...
def CallMulticall():
    ...
    txSwap = contractrouter.encodeABI(fn_name="exactInputSingle", args=[(tokenaddr, wrapped, 500, contract_router, amount, 0, 0)])
    ...
    txEncode2 = contractrouter.encodeABI(fn_name="exactInputSingle", args=[(wrapped, tokenaddr, 500, contract_router, est, 0, 0)])
    txEstimategas = [txEncode2, txWETH]
    gas_tx = contractrouter.functions.multicall(deadline, txEstimategas).buildTransaction({
        'chainId': chainId,
        'from': sender,
        'value': est,
        'gasPrice': web3.eth.gas_price, #web3.toWei(gasPrice,'gwei'),
    ...
    print('Processing Swap Sell :' ,amountFromWei ,tokenName, 'TO BNB')
