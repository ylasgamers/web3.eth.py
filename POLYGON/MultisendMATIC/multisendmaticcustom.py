...
bsc = "https://poly-rpc.gateway.pokt.network" #rpc polygon custom
web3 = Web3(Web3.HTTPProvider(bsc))

ctypes.windll.kernel32.SetConsoleTitleW("Multisender MATIC Custom")
print('multisender matic with custom amount')
...
inputamount1 = float(input("Enter amount of matic you want to send recipient1: ")) #ex 1 / 0.1 / 0.001 / 0.0001 / 0.00001
inputamount2 = float(input("Enter amount of matic you want to send recipient2: ")) #ex 1 / 0.1 / 0.001 / 0.0001 / 0.00001
amount1 = web3.toWei(float(inputamount1), 'ether')
amount2 = web3.toWei(float(inputamount2), 'ether')
amountall = [amount1, amount2] #2 address, if want more send to a lot address, you need modif it
total_pay = web3.toWei(float(inputamount1+inputamount2), 'ether') #input amount1+amount2 = totalpay [ if want more send to a lot address, you need modif it ]
...
print('https://polygonscan.com/tx/'+txid)
pc.copy('https://polygonscan.com/tx/'+txid)
