...
with open("ERC721NFT.sol", "r") as file:
    erc721nft_file = file.read()
...
compiled_sol = compile_standard(
    {
        "language": "Solidity",
        "sources": {"ERC721NFT.sol": {"content": erc721nft_file}},
        "settings": {
             "optimizer": {
             "enabled": bool(True),
             "runs": 200
...
bytecode = compiled_sol["contracts"]["ERC721NFT.sol"]["ERC721NFT"]["evm"]["bytecode"]["object"]# get abi
abi = json.loads(compiled_sol["contracts"]["ERC721NFT.sol"]["ERC721NFT"]["metadata"])["output"]["abi"]

#set title
ctypes.windll.kernel32.SetConsoleTitleW("Deploy ERC721NFT Standart On Blockchain Network")
print('')
print('Deploy ERC721NFT Standart On Blockchain Network With Python')
...
nftName_ = str(input("Enter NFT Name [Exampel CAT] : "))
nftSymbol_ = str(input("Enter NFT Symbol [Example CT] : "))
...
gas_tx = Contract.constructor(nftName_, nftSymbol_).buildTransaction(
    {
        "chainId": chain_id,
...
# build transaction
transaction = Contract.constructor(nftName_, nftSymbol_).buildTransaction(
    {
        "chainId": chain_id,
        "gas": gasAmount,
        "gasPrice": web3.eth.gas_price,
