...
ctypes.windll.kernel32.SetConsoleTitleW("Cancel Presale On Launchpad AIO Blockchain Network")
print('')
print('Cancel Presale On Launchpad Pinksale AIO Blockchain Network')
...
gas_tx = contractpresale.functions.cancel().buildTransaction({
    'chainId': chainId,
    'from': sender,
...
print('Processing Cancel Presale')

token_tx = contractpresale.functions.cancel().buildTransaction({
    'chainId': chainId,
    'from': sender,
    'gas': gasAmount,
    'gasPrice': web3.eth.gas_price, #web3.toWei(gasPrice,'gwei'),
...
print('')
print('Cancel Presale Success TX-ID Copied To Clipboard')
print(txid)
pc.copy(txid)
